...
async function main() {
  ...
  // Create experiences with Unsplash images
  const experiences = [
    {
      ...
      for (const time of times) {
        await prisma.slot.create({
          data: {
            ...
            time,
            totalSpots: 10,
            availableSpots: Math.floor(Math.random() * 8) + 2, // Random between 2-10
          },
        });
        slotCount++;
      }
  ...
  createdPromos.forEach((promo) => {
    console.log(`   - ${promo.code}: ${promo.discount}${promo.type === 'percentage' ? '%' : '₹'} off`);
  });
}
